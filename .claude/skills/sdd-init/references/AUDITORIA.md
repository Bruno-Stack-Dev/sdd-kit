...
- **Avalie a saúde do que existe** com as skills `arch-*` (pack `arch`, **inativo por padrão** —
  **ative-o antes** com `sdd pack activate arch`):
...
4. **`sdd.config.yaml`** (raiz) ← `sdd template show config`, preenchido a partir do código +
   respostas. `commands` têm de ser os **reais** (validados no Passo 1). Se o projeto já tinha um
   `sdd.config.md` v2, use `sdd config migrate` em vez de reescrever. Valide com
...
6. **Validar:** rode `sdd doctor --fast` **e o comando de testes real** — confirme que
